    ...
    internal sealed class Header
    {
            ...
            out HeaderInfo headerInfo, out DataReferenceInfo referenceInfo)
        {
...
#if !(NETSTANDARD2_0 || NET461_OR_GREATER)
            if (hInfoArr[0].AsSpan().SequenceEqual("HDIFFSF20"))
#else
            if (hInfoArr[0] == "HDIFFSF20")
#endif
            {
                isPatchDir = false;
                headerInfo.isSingleCompressedDiff = true;
                headerInfo.headerMagic = hInfoArr[0];

                if (hInfoArr.Length > 1 && !string.IsNullOrEmpty(hInfoArr[1]))
                {
                    if (!Enum.TryParse(hInfoArr[1], true, out headerInfo.compMode))
                        throw new FormatException(
                            $"[Header::TryParseHeaderInfo] This patcher doesn't support {hInfoArr[1]} compression!");
                }

                // 作为独立文件，它在包内的绝对偏移量就是 0
                referenceInfo.hdiffDataOffset = 0;
                TryReadSingleFileHeaderInfo(sr, diffPath, ref headerInfo, referenceInfo);

                HDiffPatch.Event.PushLog(
                    $"[Header::TryParseHeaderInfo] HDIFFSF20 Direct Single Compressed Diff Parsed.", Verbosity.Debug);
                return isPatchDir;
            }
            ...
            if (hInfoArr.Length == 2)
            {
                ...
                if (pFileVer != 13)
                    throw new FormatException(
                        "[Header::TryParseHeaderInfo] HDiff version is unsupported. This patcher only supports the single patch file with version: 13 only!");
                ...
                HDiffPatch.Event.PushLog(
                    $"[Header::TryParseHeaderInfo] Version: {pFileVer} Compression: {headerInfo.compMode}",
                    Verbosity.Debug);
            }
            else if (hInfoArr.Length != 3)
                throw new IndexOutOfRangeException(
                    $"[Header::TryParseHeaderInfo] Header info is incomplete! Expecting 3 parts but got {hInfoArr.Length} part(s) instead (Raw: {headerInfoLine})");
            ...
            if (isPatchDir)
            {
                ...
                if (hInfoVer != 19)
                    throw new FormatException(
                        "[Header::TryParseHeaderInfo] HDiff version is unsupported. This patcher only supports the directory patch file with version: 19 only!");

                if (hInfoArr[1] != "" && !Enum.TryParse(hInfoArr[1], true, out headerInfo.compMode))
                    throw new FormatException(
                        $"[Header::TryParseHeaderInfo] This patcher doesn't support {hInfoArr[1]} compression!");
                if (string.IsNullOrEmpty(hInfoArr[2])) headerInfo.checksumMode = ChecksumMode.nochecksum;
                else if (!Enum.TryParse(hInfoArr[2], true, out headerInfo.checksumMode))
                    throw new FormatException(
                        $"[Header::TryParseHeaderInfo] This patcher doesn't support {hInfoArr[2]} checksum!");
                HDiffPatch.Event.PushLog(
                    $"[Header::TryParseHeaderInfo] Version: {hInfoVer} ChecksumMode: {headerInfo.checksumMode} Compression: {headerInfo.compMode}",
                    Verbosity.Debug);
        ...
        private static void TryReadExternReferenceInfo(Stream sr, string diffPath, ref HeaderInfo headerInfo,
            ref DataReferenceInfo referenceInfo)
        {
            long curPos = sr.Position;
            referenceInfo.headDataOffset = curPos;

            curPos += (referenceInfo.headDataCompressedSize > 0
                ? referenceInfo.headDataCompressedSize
                : referenceInfo.headDataSize);
            ...
            HDiffPatch.Event.PushLog(
                $"[Header::TryReadExternReferenceInfo] headDataOffset: {referenceInfo.headDataOffset} | privateExternDataOffset: {referenceInfo.privateExternDataOffset} | externDataOffset: {referenceInfo.externDataOffset} | hdiffDataOffset: {referenceInfo.hdiffDataOffset} | hdiffDataSize: {referenceInfo.hdiffDataSize}",
                Verbosity.Debug);

            TryIdentifyDiffType(sr, diffPath, ref headerInfo, ref referenceInfo);
        }

        private static void TryIdentifyDiffType(Stream sr, string diffPath, ref HeaderInfo headerInfo,
            ref DataReferenceInfo referenceInfo)
        {
            ...
            HDiffPatch.Event.PushLog($"[Header::TryIdentifyDiffType] HDIFF Dir Signature: {singleCompressedHeaderLine}",
                Verbosity.Debug);

            if (singleCompressedHeaderArr[1] != "" &&
                !Enum.TryParse(singleCompressedHeaderArr[1], true, out headerInfo.compMode))
                throw new FormatException(
                    $"[Header::TryIdentifyDiffType] The compression chunk has unsupported compression: {singleCompressedHeaderArr[1]}");
            headerInfo.headerMagic = singleCompressedHeaderArr[0];

            TryReadNonSingleFileHeaderInfo(sr, diffPath, ref headerInfo);
        }

        private static void TryReadSingleFileHeaderInfo(Stream sr, string diffPath, ref HeaderInfo headerInfo,
            DataReferenceInfo referenceInfo)
        {
            ...
            HDiffPatch.Event.PushLog(
                $"[Header::TryReadSingleFileHeaderInfo] oldDataSize: {headerInfo.oldDataSize} | newDataSize: {headerInfo.newDataSize}",
                Verbosity.Debug);
            ...
            HDiffPatch.Event.PushLog(
                $"[Header::TryReadSingleFileHeaderInfo] compressedCount: {headerInfo.compressedCount}",
                Verbosity.Debug);
        }
        ...
        private static void TryReadNonSingleFileHeaderInfo(Stream sr, string diffPath, ref HeaderInfo headerInfo)
        {
            ...
            HDiffPatch.Event.PushLog(
                $"[Header::TryReadNonSingleFileHeaderInfo] oldDataSize: {headerInfo.oldDataSize} | newDataSize: {headerInfo.newDataSize}",
                Verbosity.Debug);
            ...
            HDiffPatch.Event.PushLog(
                $"[Header::TryReadNonSingleFileHeaderInfo] compressedCount: {headerInfo.compressedCount}",
                Verbosity.Debug);
        }
        ...
        private static void GetDiffChunkInfo(Stream sr, out DiffChunkInfo chunkInfo, long typeEndPos)
        {
            ...
            HDiffPatch.Event.PushLog(
                $"[Header::GetDiffChunkInfo] coverCount: {chunkInfo.coverCount} | compressSizeBeginPos: {chunkInfo.compressSizeBeginPos}",
                Verbosity.Debug);
            ...
            HDiffPatch.Event.PushLog(
                $"[Header::GetDiffChunkInfo] cover_buf_size: {chunkInfo.cover_buf_size} |  compress_cover_buf_size: {chunkInfo.compress_cover_buf_size}",
                Verbosity.Debug);
            HDiffPatch.Event.PushLog(
                $"[Header::GetDiffChunkInfo] rle_ctrlBuf_size: {chunkInfo.rle_ctrlBuf_size} | compress_rle_ctrlBuf_size: {chunkInfo.compress_rle_ctrlBuf_size}",
                Verbosity.Debug);
            HDiffPatch.Event.PushLog(
                $"[Header::GetDiffChunkInfo] rle_codeBuf_size: {chunkInfo.rle_codeBuf_size} | compress_rle_codeBuf_size: {chunkInfo.compress_rle_codeBuf_size}",
                Verbosity.Debug);
            HDiffPatch.Event.PushLog(
                $"[Header::GetDiffChunkInfo] newDataDiff_size: {chunkInfo.newDataDiff_size} | compress_newDataDiff_size: {chunkInfo.compress_newDataDiff_size}",
                Verbosity.Debug);
            ...
            HDiffPatch.Event.PushLog(
                $"[Header::GetDiffChunkInfo] headEndPos: {chunkInfo.headEndPos} | coverEndPos: {chunkInfo.coverEndPos}",
                Verbosity.Debug);
        }

        private static void TryReadHeaderAndReferenceInfo(Stream sr, ref HeaderInfo headerInfo,
            ref DataReferenceInfo referenceInfo)
        {
            ...
            HDiffPatch.Event.PushLog(
                $"[Header::TryReadHeaderAndReferenceInfo] Is In/Out a Dir -> Input: {headerInfo.isInputDir} / Output: {headerInfo.isOutputDir}",
                Verbosity.Debug);
            ...
            HDiffPatch.Event.PushLog(
                $"[Header::TryReadHeaderAndReferenceInfo] InDir Count/SumSize: {referenceInfo.inputDirCount}/{referenceInfo.inputSumSize} | OutDir Count/SumSize: {referenceInfo.outputSumSize}/{referenceInfo.inputSumSize}",
                Verbosity.Debug);
            ...
            HDiffPatch.Event.PushLog(
                $"[Header::TryReadHeaderAndReferenceInfo] InRef Count/Size: {referenceInfo.inputRefFileCount}/{referenceInfo.inputRefFileSize} | OutRef Count/Size: {referenceInfo.outputRefFileCount}/{referenceInfo.outputRefFileSize}",
                Verbosity.Debug);
            ...
            HDiffPatch.Event.PushLog(
                $"[Header::TryReadHeaderAndReferenceInfo] IdenticalPair Count/Size: {referenceInfo.sameFilePairCount}/{referenceInfo.sameFileSize}",
                Verbosity.Debug);
            ...
            HDiffPatch.Event.PushLog(
                $"[Header::TryReadHeaderAndReferenceInfo] newExecuteCount: {referenceInfo.newExecuteCount} | privateReservedDataSize: {referenceInfo.privateReservedDataSize} | privateExternDataSize: {referenceInfo.privateExternDataSize} | privateExternDataSize: {referenceInfo.externDataSize}",
                Verbosity.Debug);
            ...
            HDiffPatch.Event.PushLog(
                $"[Header::TryReadHeaderAndReferenceInfo] compressSizeBeginPos: {referenceInfo.compressSizeBeginPos} | headDataSize: {referenceInfo.headDataSize} | headDataCompressedSize: {referenceInfo.headDataCompressedSize} | checksumByteSize: {referenceInfo.checksumByteSize}",
                Verbosity.Debug);
            HDiffPatch.Event.PushLog(
                $"[Header::TryReadHeaderAndReferenceInfo] checksumOffset: {referenceInfo.checksumOffset} | dirDataIsCompressed: {headerInfo.dirDataIsCompressed}",
                Verbosity.Debug);

            if (referenceInfo.checksumByteSize > 0)
            {
                HDiffPatch.Event.PushLog(
                    $"[Header::TryReadHeaderAndReferenceInfo] Seeking += {referenceInfo.checksumByteSize * 4} bytes from checksum bytes!",
                    Verbosity.Debug);
        ...
        private static void TrySeekHeader(Stream sr, int skipLongSize)
        {
            ...
            HDiffPatch.Event.PushLog(
                $"[Header::TrySeekHeader] Seeking from: {sr.Position} += {skipLongSize} to {sr.Position + skipLongSize}",
                Verbosity.Debug);
        ...
        private static byte TryGetVersion(string str)
        {
            ...
            if (lastIndexOf < 0)
                throw new IndexOutOfRangeException(
                    $"[Header::TryGetVersion] Version string is invalid! Cannot find the matching start of \"HDIFF\". Getting: {str} instead");

            string numStr = str.Substring(lastIndexOf + HDIFF_HEAD.Length);
            if (byte.TryParse(numStr, out byte ret)) return ret;

            throw new InvalidDataException(
                $"[Header::TryGetVersion] Version string is invalid! Value: {numStr} (Raw: {str})");
        }
#endif
    }
}