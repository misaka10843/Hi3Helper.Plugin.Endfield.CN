...
namespace SharpHDiffPatch.Core.Patch
{
    ...
    {
        internal unsafe delegate void RleProc(ref RleRefClipStruct rleLoader, MemoryStream outCache,
            ref long copyLength, int decodeStep, byte* rlePtr, byte[] rleBuffer, int rleBufferIdx, byte* oldPtr);
        ...
        static unsafe PatchCore()
        {
            RleProcDelegate =
#if !(NETSTANDARD2_0 || NET461_OR_GREATER)
                Sse2.IsSupported ? TBytesSetRleVectorSse2Simd :
#if NET7_0_OR_GREATER
                AdvSimd.IsSupported && Vector128.IsHardwareAccelerated ? TBytesSetRleVectorAdvSimd128 :
                AdvSimd.IsSupported && Vector64.IsHardwareAccelerated ? TBytesSetRleVectorAdvSimd64 :
#else
                    AdvSimd.IsSupported ?
        ...
        internal PatchCore(long sizeToBePatched, Stopwatch stopwatch, string inputPath, string outputPath,
            Action<long>? writeBytesDelegate, CancellationToken token)
        {
            Token = token;
            ...
            long start, long length, long compLength, out long outLength, bool isBuffered, bool isFastBufferUsed)
        {
            ...
            CompressionStreamHelper.GetDecompressStreamPlugin(compMode, sourceStream, out Stream returnStream, length,
                compLength, out outLength, isBuffered);

            if (!isBuffered || isFastBufferUsed) return returnStream;
            HDiffPatch.Event.PushLog(
                $"[PatchCore::GetBufferStreamFromOffset] Caching stream from offset: {start} with length: {(compLength > 0 ? compLength : length)}");
            using (returnStream)
            {
        ...
        public void UncoverBufferClipsStream(Stream[] clips, Stream inputStream, Stream outputStream,
            HeaderInfo headerInfo) => WriteCoverStreamToOutput(clips, inputStream, outputStream,
            headerInfo.chunkInfo.coverCount, headerInfo.chunkInfo.cover_buf_size, headerInfo.newDataSize);

        internal static IEnumerable<CoverHeader> EnumerateCoverHeaders(Stream coverReader, long coverSize,
            long coverCount)
        {
            ...
            {
                HDiffPatch.Event.PushLog(
                    $"[PatchCore::EnumerateCoverHeaders] Enumerate cover counts from buffer with size: {coverSize}",
                    Verbosity.Verbose);
            ...
            {
                HDiffPatch.Event.PushLog(
                    $"[PatchCore::EnumerateCoverHeaders] Enumerate cover counts directly from stream (Size: {coverSize}).",
                    Verbosity.Verbose);
                while (coverCount-- > 0)
                {
                    long oldPosBack = lastOldPosBack;
        ...
        internal void RunCopySimilarFilesRoutine()
        {
            ...
            HDiffPatch.Event.PushLog(
                $"Copying similar data has been finished in {timeTaken.TotalSeconds} seconds ({timeTaken.TotalMilliseconds} ms)");
            HDiffPatch.Event.PushLog("Starting patching process...");
            Stopwatch.Restart();
        }

        private void WriteCoverStreamToOutput(Stream[] clips, Stream inputStream, Stream outputStream, long coverCount,
            long coverSize, long newDataSize)
        {
            ...
            try
            {
                ...
                for (int i = 0; i < headers.Length; i++)
                {
                    ...
                    if (newPosBack < cover.NewPos)
                    {
                        ...
                        TBytesDetermineRleType(ref rleStruct, cacheOutputStream, copyLength, sharedBuffer, clips[1],
                            clips[2]);
                    }

                    TBytesCopyOldClipPatch(cacheOutputStream, inputStream, ref rleStruct, cover.OldPos,
                        cover.CoverLength, sharedBuffer, clips[1], clips[2]);
        ...
        internal void SpawnCorePatchFinishedMsg()
        {
            TimeSpan timeTaken = Stopwatch.Elapsed;
            HDiffPatch.Event.PushLog(
                $"Patching has been finished in {timeTaken.TotalSeconds} seconds ({timeTaken.TotalMilliseconds} ms)");
        }

        private static void TBytesCopyOldClipPatch(MemoryStream outCache, Stream inputStream,
            ref RleRefClipStruct rleLoader, long oldPos, long addLength, byte[] sharedBuffer,
        ...
        internal static void TBytesCopyStreamFromOldClip(MemoryStream outCache, Stream copyReader, long copyLength,
            byte[] sharedBuffer)
        {
            long lastPos = outCache.Position;
        ...
        private static unsafe void TBytesSetRle(ref RleRefClipStruct rleLoader, MemoryStream outCache,
            ref long copyLength, byte[] sharedBuffer, Stream rleCodeStream)
        {
        ...
        internal static void TBytesSetRleSingle(ref RleRefClipStruct rleLoader, MemoryStream outCache,
            ref long copyLength, byte[] sharedBuffer)
        {
        ...
        internal static unsafe void TBytesSetRleVectorSoftware(ref RleRefClipStruct rleLoader, MemoryStream outCache,
            ref long copyLength, int decodeStep, byte* rlePtr, byte[] rleBuffer, int rleBufferIdx, byte* oldPtr)
        {
            int index = 0;
...
#if !(NETSTANDARD2_0 || NET461_OR_GREATER)
        internal static unsafe void TBytesSetRleVectorAdvSimd128(ref RleRefClipStruct rleLoader, MemoryStream outCache,
            ref long copyLength, int decodeStep, byte* rlePtr, byte[] rleBuffer, int rleBufferIdx, byte* oldPtr)
        {
            int len = decodeStep;
            ...
            if (len >= Vector128<byte>.Count)
            {
                ...
                Vector128<byte> resultVector =
                    AdvSimd.Add(*(Vector128<byte>*)(rlePtr + len), *(Vector128<byte>*)(oldPtr + len));
                AdvSimd.Store(rlePtr + len, resultVector);
                if (len >= Vector128<byte>.Count) goto AddVectorArm64_128;
            }

            WriteRemainedRleSimdResultToStream(ref rleLoader, len, outCache, ref copyLength, decodeStep, rlePtr,
                rleBuffer, rleBufferIdx, oldPtr);
        }


        internal static unsafe void TBytesSetRleVectorAdvSimd64(ref RleRefClipStruct rleLoader, MemoryStream outCache,
            ref long copyLength, int decodeStep, byte* rlePtr, byte[] rleBuffer, int rleBufferIdx, byte* oldPtr)
        {
            int len = decodeStep;
            ...
            if (len >= Vector64<byte>.Count)
            {
                ...
                Vector64<byte> resultVector =
                    AdvSimd.Add(*(Vector64<byte>*)(rlePtr + len), *(Vector64<byte>*)(oldPtr + len));
                AdvSimd.Store(rlePtr + len, resultVector);
        ...
        internal static unsafe void TBytesSetRleVectorSse2Simd(ref RleRefClipStruct rleLoader, MemoryStream outCache,
            ref long copyLength, int decodeStep, byte* rlePtr, byte[] rleBuffer, int rleBufferIdx, byte* oldPtr)
        {
            int len = decodeStep;
            ...
            if (len >= Vector128<byte>.Count)
            {
                ...
                len -= Vector128<byte>.Count;
                Vector128<byte> resultVector =
                    Sse2.Add(*(Vector128<byte>*)(rlePtr + len), *(Vector128<byte>*)(oldPtr + len));
        ...
        private static unsafe void WriteRemainedRleSimdResultToStream(ref RleRefClipStruct rleLoader, int len,
            MemoryStream outCache, ref long copyLength, int decodeStep, byte* rlePtr, byte[] rleBuffer,
            int rleBufferIdx, byte* oldPtr)
        {
            if (len >= 4)
            {
                AddRemainsFourStep:
        ...
        private static void WriteRleResultToStream(ref RleRefClipStruct rleLoader, MemoryStream outCache,
            byte[] rleBuffer, int rleBufferIdx, ref long copyLength, int decodeStep)
        {
        ...
        private void CopyOldSimilarToNewFiles(DirectoryReferencePair dirData)
        {
            ...
            while (curPathIndex < pathCount)
            {
                if ((curNewRefIndex < newRefCount)
                    && (curPathIndex == (dirData.NewRefList.Length > 0
                        ? (int)dirData.NewRefList[curNewRefIndex]
                        : curNewRefIndex)))
                {
                ...
                else
                {
                    ...
                    if (pathByIndex.Length > 0)
                    {
                        ...
                        if (isPathADir && !Directory.Exists(combinedNewPath))
                            Directory.CreateDirectory(combinedNewPath);
                        else if (!isPathADir && !File.Exists(combinedNewPath)) File.Create(combinedNewPath).Dispose();
                    }

                    HDiffPatch.Event.PushLog(
                        $"[PatchCore::CopyOldSimilarToNewFiles] Created a new {(isPathADir ? "directory" : "empty file")}: {combinedNewPath}",
                        Verbosity.Debug);
        ...
        private void CopyFileByPairIndex(string[] oldList, string[] newList, long oldIndex, long newIndex)
        {
            ...
            HDiffPatch.Event.PushLog(
                $"[PatchCore::CopyFileByPairIndex] Copying similar file to target path: {oldFullPath} -> {newFullPath}",
                Verbosity.Debug);
